...
@implementation UIButton (leCustom)

// 备注：在button多个状态同时请求网络url时候，去掉cancelCurrentImageLoad 方法，否则normal状态会被select 状态覆盖掉，为解决频道墙button normal和hightligt状态的方法，其他地方慎重调用
- (void)setImageWithURLWithOutCancel:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options completed:(SDWebImageCompletionBlock)completedBlock {
    [self setImage:placeholder forState:state];
    if (url) {
